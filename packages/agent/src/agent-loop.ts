		...
		while (hasMoreToolCalls || pendingMessages.length > 0) {
			...
			if (config.policy) {
				...
				if (policyDecision.abort) {
					...
					await emit({ type: "message_start", message: abortMessage });
					await emit({ type: "message_end", message: abortMessage });
					await emit({ type: "turn_end", message: abortMessage, toolResults: [] });
			...
			if (policyDecision?.shouldCompact && config.onCompact) {
				try {
					...
					await emit({ type: "message_start", message: compactErrorMessage });
					await emit({ type: "message_end", message: compactErrorMessage });
					await emit({ type: "turn_end", message: compactErrorMessage, toolResults: [] });
					await emit({ type: "agent_end", messages: newMessages });
					return;
				}
			}
			...
			const message = await streamAssistantResponse(currentContext, config, signal, emit, streamFn, policyDecision);
			newMessages.push(message);
